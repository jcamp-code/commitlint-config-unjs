...
async function commitLint(message) {
  // @ts-expect-error types difference
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const preset = await require(Configuration.parserPreset)()

  // @ts-expect-error types difference
  return lint(message, Configuration.rules, { ...preset })
}
...
const errors = {
  typeEnum: {
    level: 2,
    message:
      'type must be one of [build, chore, ci, docs, examples, feat, fix, perf, refactor, revert, style, test, types, wip]',
    name: 'type-enum',
    valid: false,
  },
  typeCase: {
